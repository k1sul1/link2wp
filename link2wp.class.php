...
class Link2WP {
  ...
  public function parsePage($page_contents,$bodypart = "body", $titlepart = 'title'){
    ...
    $parsed_page->content = $crawler->html();
  ...
  public static function getComponentText($page_contents,$component_selector = null){
    if(is_null($component_selector)) throw new Exception("Component selector cannot be null!", 1);

    $crawler = new Symfony\Component\DomCrawler\Crawler($page_contents);

    $component = $crawler->filter($component_selector)->eq(0)->text();



    if(self::$debug_mode){
      echo "Component text:".PHP_EOL;
      print_r($component);
    }
  ...
  public function createPost($title = "(no title)", $content = "(no content)",$opt = array()){
    ...
    if(function_exists("link2wp_createpost_callback")){

      $args = array("postID" => $post_id,"misc" => $p);
      call_user_func("link2wp_createpost_callback", $args); // possible to create post meta
    }
  ...
  public function updatePost($id = null, $title = "(no title)", $content = "(no content)", $opt = array()){
    ...
    if(function_exists("link2wp_updatepost_callback")){
      call_user_func_array("link2wp_updatepost_callback", array($post_id,$p)); // possible to create post meta
    }
  ...
  public function init($links = array(), $crawl_options, $update = false, $opt = array()){
    ...
    foreach($links as $id => $link){
      $page_contents = self::fetchPage($link);
      ...
      $opt['link2wp_original_link'] = $link;
  ...
  public static function fetchPage($url,$timeout = 10){
